...
use crate::api::models::{InspectionTask, InspectionTasksNoteAttrValVO, TaskPunishNoticeReq};
...
pub fn PunishNoticePage() -> impl IntoView {
    ...
    let cases = vec![
        InspectionTask {
            ...
            inspection_name: Some("拟行政处罚告知 - 湖南省人民医院".to_string()),
    ...
    let handle_submit = move |_| {
        ...
        let req = TaskPunishNoticeReq {
            inspection_id: task_id as i32,
            reqs: reqs_data.get(),
        };
        leptos::logging::log!("Submitting TaskPunishNoticeReq: {:?}", req);
        let _ = window().alert_with_message("已完成告知环节确认！");
